...
export default function BuildTimelineShowcase() {
  ...
  const phases: Phase[] = [
    ...
    },
    {
      ...
      specs: ['Civil Defense approved loops', 'DoE electrical installation certs', 'HVAC chiller balancing done'],
      refCode: 'MEP-HVAC-99'
    },
    {
      num: '05',
        ...
        {phases.map((phase, idx) => {
          ...
          return (
            ...
            >
              {/* Circular Node */}
              <div
                className={`w-8 h-8 rounded-full border-2 flex items-center justify-center font-mono text-[10px] font-bold transition-all duration-300 ${isActive
              ...
              <span className={`hidden md:block absolute top-10 whitespace-nowrap text-[9px] font-mono uppercase tracking-wider font-bold transition-colors ${isActive ? 'text-brand-teal' : 'text-app-muted'
                }`}>
                {phase.name.split(' ')[0]}
              </span>
            </button>
